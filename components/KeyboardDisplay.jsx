...
const KeyboardDisplay = ({ selectedButtons, selectionOrder }) => {
  const getSelectedColor = (num) => {
    ...
    return selectedButtons.includes(num)
  ...
  const KeyButton = ({ num, isTop }) => (
    <div
      className={`flex items-center justify-center ${
        isTop ? "h-7" : "h-8"
      } w-6 outline outline-2 ${
        isTop ? "outline-sky-200" : "outline-sky-500"
      } rounded-sm ${getSelectedColor(num)} 
        ${num === "4" && isTop ? "mr-7" : ""}`}
    >
      {selectedButtons.includes(num) && (
        <p className="text-neutral-950 text-sm font-bold">
          {getSelectionNumber(num)}
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 bg-sky-950 h-28 px-6 rounded-lg col-start-2 col-span-7 ">
